<template>
  <header>
    <nav class="max-w-[1260px] flex flex-wrap items-center justify-between mx-auto py-5 sm:py-8 text-lg tracking-wide">
      <router-link to="/" @click="hideMenu" class="flex items-center space-x-3 rtl:space-x-reverse">
        <LogoBlock :logoClass="'w-28 sm:w-44'" />
      </router-link>
      <button @click="toggle" type="button" class="items-right p-2 w-10 h-10 md:hidden mt-2"
        aria-controls="navbar-default" aria-expanded="false">
        <span class="sr-only">Menu</span>
        <svg v-if="!open" class="w-full text-right" viewBox="0 0 27 19" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M1.125 9.5H25.875M1.125 1.25H25.875M9.375 17.75H25.875" stroke="white" stroke-width="2"
            stroke-linecap="round" stroke-linejoin="round" />
        </svg>

        <svg v-if="open" class="w-full" viewBox="0 0 33 33" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M6.875 16.6913H26.125" stroke="white" stroke-width="2" stroke-linecap="round"
...
export default {
  ...
  setup() {
    ...
    return {
      ...
      hideMenu // Add the hideMenu function to the returned object
    };
  },
};
</script>
